  ...
  return (
    <section className="bg-white dark:bg-gray-900 py-16">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <p className="text-center text-shadow-gray dark:text-gray-400 mb-10">Find what you need from fellow Colgate students</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {categories.map((cat) => (
            <Link
              ...
              className="group border border-winter-gray dark:border-gray-700 rounded-xl p-6 flex flex-col items-center text-center hover:border-maroon hover:shadow-md transition-all cursor-pointer"
            >
              <span className="text-4xl mb-3">{cat.emoji}</span>
              <span className="font-semibold text-gray-900 dark:text-gray-100 group-hover:text-maroon transition-colors">{cat.label}</span>
              <span className="text-sm text-shadow-gray dark:text-gray-400 mt-1">{cat.desc}</span>
            </Link>
          ))}
        </div>
      </div>
